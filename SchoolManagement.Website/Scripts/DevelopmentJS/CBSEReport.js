    ...
    $("#BtnDownloadAll").on('click', function () {
        ...
        if (missingField !== "") {
            ...
            $.ajax({
                ...
                dataType: "json",
                contentType: "application/json;charset=UTF-8",
                data: { classId: classid, sectionId: sectionid, TermId: 1, Batchid: batchid },
                success: function (result) {
                    console.log(result)
                    $.each(result, function (key, data) {
                        //if (count < 5) {
...
async function fetchData(rollno, number) {
    ...
    const BacthId = localStorage.getItem("PrintTerm");
    ...
    fetch('/Exam/PrintReportCardCBSEBoard?studentId=' + number + '&termId=' + parseInt(splitValues.id) + '&BatchId=' + parseInt(splitValues.id), {
        ...
        .then(data => {
            if (data.subjectDatas && Array.isArray(data.subjectDatas)) {
                ...
                var schoolLogo1ImgUrl = currentUrl + "/Content/Images/fevicon1.jpg";
                var schoolLogo2ImgUrl = currentUrl + "/Content/Images/logo3.png";
                var principleSign = currentUrl + "/Content/Images/principle.png";

                $("#principleSign").attr("src", principleSign);
                $("#schoolLogo1").attr("src", schoolLogo1ImgUrl);
                $("#schoolLogo2").attr("src", schoolLogo2ImgUrl);
                ...
                subjectDatas.forEach(subjectData => {
                    ...
                    $("#UT1MaxMarkTotal").text("(" + (subjectData.MaxMarksUT1) + ")");
                    $("#UT2MaxMarkTotal").text("(" + (subjectData.MaxMarksUT2) + ")");

                    $(".Term1TheoryMaxMarkTotal").text("(" + (subjectData.MaxMarksTerm1Theory) + ")");
                    $("#Term1PracticalMaxMarkTotal").text("(" + (subjectData.MaxMarksTerm1Practical) + ")");
                    $("#Term2TheoryMaxMarkTotal").text("(" + (subjectData.MaxMarksTerm2Theory) + ")");
                    $("#Term2PracticalMaxMarkTotal").text("(" + (subjectData.MaxMarksTerm2Practical) + ")");

                    $("#UTTotalMaxMarkTotal").text("(" + ((parseInt(subjectData.MaxMarksUT1) + parseInt(subjectData.MaxMarksUT2))) + ")");
                    $("#Term1TotalMaxMarkTotal").text("(" + ((parseInt(subjectData.MaxMarksTerm1Theory) + parseInt(subjectData.MaxMarksTerm1Practical))) + ")");
                    $("#Term2TotalMaxMarkTotal").text("(" + ((parseInt(subjectData.MaxMarksTerm2Theory) + parseInt(subjectData.MaxMarksTerm2Practical))) + ")");
                    return;
                });
                ...
                var imageUrl = currentUrl + "/WebsiteImages/StaffSignature/" + data.staffSignatureLink;
                ...
                //        $("#UTMaxMarkTotal").text("(" + (subjectData.MaxMarksUT1) + ")");
                ...
                //For Unit 2
                    ...
                    subjectDatas.forEach(subjectData => {
                        ...
                        $("#UTMaxMarkTotal").text("(" + (subjectData.MaxMarksUT2) + ")");
                ...
                //For Term 1
                else if (splitValues.id === "3" || splitValues.id === "1" /*&& splitValues.name === "Term"*/) {
                    $("#teacherRemark").hide();
                    $("#UnitTestRemark").show();
                    $("#UnitTestRemarkText").text(data.Remark);
                    //$(".Term1TheoryMaxMarkTotal").text("(" + (data.totalResult.Term1TheoryMaxTotal / subjectDataLength) + ")");
                    ...
                    subjectDatas.forEach(subjectData => {
                        ...
                        $(".Term1TheoryMaxMarkTotal").text("(" + (subjectData.MaxMarksTerm1Theory) + ")");
                        $("#Term1TPracticalMaxMarkTotal").text("(" + (subjectData.MaxMarksTerm1Practical) + ")");
                        $("#Term1TAllTotalMaxMarkTotal").text("(" + ((parseInt(subjectData.MaxMarksTerm1Theory) + parseInt(subjectData.MaxMarksTerm1Practical))) + ")");
                        return;
                    });
                    ...
                    subjectDatas.forEach(subjectData => {
                        ...
                        $(".Term1TheoryMaxMarkTotal").text("(" + (subjectData.MaxMarksTerm2Theory) + ")");
                        $("#Term1PracticalMaxMarkTotal").text("(" + (subjectData.MaxMarksTerm2Practical) + ")");
                        $("#Term1TotalMaxMarkTotal").text("(" + ((parseInt(subjectData.MaxMarksTerm2Theory) + parseInt(subjectData.MaxMarksTerm2Practical))) + ")");
                        return;
                    });
                    ...
                    subjectDatas.forEach(subjectData => {
                        ...
                        $(".Term1TheoryMaxMarkTotal").text("(" + (subjectData.MaxMarksPre1Theory) + ")");
                        $("#Term1TPracticalMaxMarkTotal").text("(" + (subjectData.MaxMarksPre1Practical) + ")");
                        $("#Term1TAllTotalMaxMarkTotal").text("(" + ((parseInt(subjectData.MaxMarksPre1Theory) + parseInt(subjectData.MaxMarksPre1Practical))) + ")");
                        return;
                    });
                    ...
                    subjectDatas.forEach(subjectData => {
                        ...
                        $(".Term1TheoryMaxMarkTotal").text("(" + (subjectData.MaxMarksPre2Theory) + ")");
                        $("#Term1TPracticalMaxMarkTotal").text("(" + (subjectData.MaxMarksPre2Practical) + ")");
                        $("#Term1TAllTotalMaxMarkTotal").text("(" + ((parseInt(subjectData.MaxMarksPre2Theory) + parseInt(subjectData.MaxMarksPre2Practical))) + ")");
                        return;
                    });
                    var tableBodyPreBoard1 = document.getElementById('PreBoardRecord').getElementsByTagName('tbody')[0];
                    // Get all rows from the table body
