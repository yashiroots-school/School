...
async function fetchData(rollno, number) {
    ...
    //New
    const reportCardResponse = await fetch('/Exam/PrintReportCardDataForCBSEResult?studentId=' + number + '&termId=' + _Stid + '&BatchId=' + BatchId, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        }
    ...
    await fetchNew(reportCardData, rollno, id, splitValues, _Stid);

}
function getGradeforSubject(percent)
{
    var grade = "";

    $.ajax({
        url: "/Home/GetGradeByPercentage",
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify(percent),
        //async: false, // Making it synchronous to return value directly
...
function getGradeforSubject(grade, classId, termid) {
    return new Promise((resolve, reject) => {
        $.ajax({
            //url: "/Exam/GetGradeByPercentage?percentage=" + percent,
            url: "/Exam/GetDescriptionByGrade",
            data: { Grade: grade, classId: classId, termid: termid },
            success: function (response) {
                //console.log(response);  // Log the response from the server
                const GradeDescription = response.GradeDescription;  // Extract the grade from the response
                //console.log(GradeDescription);  // Log the grade
                resolve(GradeDescription);  // Resolve the Promise with the grade
            },
            error: function () {
...
//function getGradeDescription(grade, classId, termid) {
//    $.ajax({
//        //url: "/Exam/GetGradeByPercentage?percentage=" + percent,
//        url: "/Exam/GetDescriptionByGrade",
//        data: { Grade: grade, classId: classId, termid: termid },
...
//            const GradeDescription = response.GradeDescription;  // Extract the grade from the response
//            //console.log(GradeDescription);  // Log the grade
//            return GradeDescription;  // Resolve the Promise with the grade
//        },
//        error: function () {
//            return "";
//           // reject("Error");  // Reject the Promise with an error message
...
//}
function getGradeDescription(grade, classId, termid) {
    var result = ""; // Variable to store response

    $.ajax({
        url: "/Exam/GetDescriptionByGrade",
        data: { Grade: grade, classId: classId, termid: termid },
        async: false, // ⛔ This makes the request synchronous (blocks UI)
        success: function (response) {
            result = response.GradeDescription; // Store the response in the variable
        },
        error: function () {
            result = "Error retrieving grade"; // Handle errors
        }
    });

    return result; // Return the result (now works synchronously)
}


function getAttendaceCount(StudnetId) {
    var Attendance = "0/0";
    return new Promise((resolve, reject) => {
        $.ajax({
            //url: "/Exam/GetGradeByPercentage?percentage=" + percent,
            url: "/Exam/GeAttendance",
            data: { StudnetId: StudnetId },
            success: function (response) {
                console.log(response);  // Log the response from the server
                if (response.Attendance) {
                    let { PresentDays, TotalDays } = response.Attendance;
...
async function fetchNew(data, rollno, id, splitValues, _Stid) {
    if (data.subjectDatas && Array.isArray(data.subjectDatas)) {
        ...
        var OptionalGradeTB = document.getElementById('OptionalGrade').getElementsByTagName('tbody')[0];
        ...
        if (data.className == "L.K.G" || data.className == "H.K.G" || data.className == "Class-Nursery") {
            $("#Ut1Name").text("UT I");
            $("#Ut2Name").text("UT II");
        }
        else
        ...
        $("#RollNo").text(rollno);
        if (data.ProfileAvatar != "" || data.ProfileAvatar != null)
            var profileurl = "/WebsiteImages/StudentPhoto/" + data.ProfileAvatar;
        $("#studentlogo").attr("src", profileurl);
        ...
        let Attendance = await getAttendaceCount(data.studentID);
        ...
        subjectDatas.forEach(subjectData => {
            ...
            subjectTd.textContent = subjectData.Subject + "(" + subjectData.MaxMarksTerm1Theory + "/" + subjectData.MaxMarksTerm1Practical + ")";
           ...
           /* */
            ...
            //newRow.innerHTML += '<td>' + checkForZero(subjectData.ProjectMarks) + '</td>';
            //newRow.innerHTML += '<td>' + checkForZero(subjectData.SubjectEnrichMarks) + '</td>';
            ...
            //newRow.innerHTML += '<td>' + checkForZero(subjectData.ProjectMarksUT2) + '</td>';
            //newRow.innerHTML += '<td>' + checkForZero(subjectData.SubjectEnrichMarksUT2) + '</td>';
       ...
       /* totalRow.innerHTML += '<td><b>' + checkForZero(data.totalResult.UTAllTotal) + '</b></td>';*/
        ...
        //totalRow.innerHTML += '<td><b>' + checkForZero(data.totalResult.ProjectTotalT1) + '</b></td>';
        //totalRow.innerHTML += '<td><b>' + checkForZero(data.totalResult.SubjectEnrichTotalT1) + '</b></td>';
        ...
        //totalRow.innerHTML += '<td><b>' + checkForZero(data.totalResult.ProjectTotalT2) + '</b></td>';
        //totalRow.innerHTML += '<td><b>' + checkForZero(data.totalResult.SubjectEnrichTotalT2) + '</b></td>';
        ...
        var percentagesRow = createAdditionalRow("Percentage", data.totalResultPercentage.UT1Total + "%",
            ...
            //data.totalResultPercentage.ProjectTotalT1 + "%",
            //data.totalResultPercentage.SubjectEnrichTotalT1 + "%",
            ...
            //data.totalResultPercentage.ProjectTotalT2 + "%",
            //data.totalResultPercentage.SubjectEnrichTotalT2 + "%",
        ...
        if (data.Remark == null || data.Remark== "")
        {
            ...
            var Grade = data.totalResultPercentage.OverallGrade;
            var ClassId = data.classID;
            let Remark = await getGradeforSubject(Grade, ClassId, _Stid);

           
            $("#teacherRemarkText").text(Remark);
        }
        else
        {
        ...
        data.optionalSubjectDatas.forEach(optionalSubjectData => {
            ...
            var A = getGradeDescription(getOptionalGradeForAllReports(optionalSubjectData.PracticalMarks), data.classID, _Stid);
            //var A =  getGradeforSubject(getOptionalGradeForAllReports(optionalSubjectData.PracticalMarks), data.classID, _Stid);
            var B = getGradeDescription(getOptionalGradeForAllReports(optionalSubjectData.PracticalMarksUT2), data.classID, _Stid);
            console.log(A,B)
            ...
            OptionalGradeTB.appendChild(moralScienceRowUT1);
        });
        ...
        //For Term 1
            ...
            for (let i = 0; i < subjectDatas.length; i++) {
                ...
                $("#Term1TotalMaxMarkTotal").text("(" + ((parseInt(subjectDatas[0].MaxMarksTerm1Theory) + parseInt(subjectDatas[0].MaxMarksUT2) + parseInt(subjectDatas[0].MaxMarksTerm1Practical))) + ")");
                $("#Term2TotalMaxMarkTotal").text("(" + ((parseInt(subjectDatas[0].MaxMarksTerm2Theory) + parseInt(subjectDatas[0].MaxMarksUT2) + parseInt(subjectDatas[0].MaxMarksTerm2Practical))) + ")");
                break;
            }
...
function createScript() {
    var script = `<script src="https://code.jquery.com/jquery-3.7.0.slim.js" integrity="sha256-7GO+jepT9gJe9LB4XFf8snVOjX3iYNb0FHYr5LI1N5c=" crossorigin="anonymous"></script>
        ...
        <script src="~/Scripts/DevelopmentJS/PrintReportCBSE.js"></script>
`
