...
namespace SchoolManagement.Website.Controllers
{
    public class PaymentController : Controller
    {
        ...
        public ActionResult PaymentProcess()
        {
            ...
            //var Feeheadinglist = _context.FeeHeadings.ToList();
            //string[] feeheadings = objPaymentViewModels.FeeHeadings.Split(',');
            //if(feeheadings.Length > 0)
            //{
            //    foreach(var item in feeheadings)
            //    {
            //        var feeheadname = Feeheadinglist.FirstOrDefault(x => x.FeeName == item);
            ...
            if (objPaymentViewModels != null)
            {
                ...
                //if (classdetails != null && classdetails.ToString() != "207" || classdetails.ToString() != "208" || classdetails.ToString() != "209" || classdetails.ToString() != "210" && objPaymentViewModels.FeeHeadings != "Transport Fee")
                //{
                //    //CARMEL TERESA SCHOOL
                //    objInputPayment.Mid = ConfigurationManager.AppSettings["SchoolPaymentWID"];
                //    objInputPayment.Enckey = ConfigurationManager.AppSettings["SchoolPaymentkey"];
                ...
                //    objInputPayment.Enckey = ConfigurationManager.AppSettings["SchoolTransportkey"];
                //}
                //else if (objPaymentViewModels.FeeHeadings != null && objPaymentViewModels.FeeHeadings == "Transport Fee")
                //{
                //    objInputPayment.Mid = ConfigurationManager.AppSettings["SchoolTransportWID"];
                //    objInputPayment.Enckey = ConfigurationManager.AppSettings["SchoolTransportkey"];
                //    objInputPayment.ResponseUrl = ConfigurationManager.AppSettings["TrabsportUrl"];
                ...
                //objInputPayment.AddField10 = Convert.ToString( ObjPaymentResultModels.ConcessionAmt);
                //Session["DiscountAmount"] = Convert.ToString(ObjPaymentResultModels.ConcessionAmt);
                //var discountdetails = Session["DiscountAmount"]; 
        ...
        public ActionResult PaymentProcessResult()
        {
            ...
            if (Request.Form["merchantResponse"] != null)
            {
                ...
                if (objResMsgDTO.StatusCode == "S")
                {
                    ...
                    if (feeid != "")
                    {
                        var studentdata = _context.Students.FirstOrDefault(x => x.StudentId == stuid);
                        if (studentdata != null)
                        {
                            if (studentdata.ParentEmail != null)
                            {
                                ...
                                //For Receipt
                                var str = SendEmail("" + studentdata.ParentEmail + "", "Admission Fee Payment Receipt", "" + studentname + " You can Download your Admission Fee Receipt through this Link" + link + "");
                            }
                        }
                    }
                    ///

                    return Content("<script language='javascript' type='text/javascript'>window.location.href='AdmissionFee/FeeReceipts';alert('Transaction Completed Successfully!, Transaction Reference: " + objPaymentStatus.PgMeTrnRefNo + ", Transaction Amount:" + objPaymentStatus.TrnAmt + ", Transaction Request Date:" + objPaymentStatus.TrnReqDate + ", Transaction Status:" + objPaymentStatus.StatusCode + ", Transaction Description" + objPaymentStatus.StatusDesc + ", Student Name:" + objPaymentStatus.AddField1 + ", Class:" + objPaymentStatus.AddField2 + "')</script>");
        ...
        public ActionResult PaymentProcessSchoolResult()
        {
            try
            {
                ...
                if (Request.Form["merchantResponse"] != null)
                {
                    ...
                    objPaymentStatus.Rrn = objResMsgDTO.Rrn;
                    objPaymentStatus.OrderId = objResMsgDTO.OrderId;
                    //var ConsessionAmount = Session["DiscountAmount"];
                    string Status = string.Empty;
                    if (objResMsgDTO.StatusCode == "S")
                    {
                        Status = "Success";
        ...
        public string genericFeeCalculator(InputPaymentResult objPaymentStatus, int stuid)
        {
            try
            {
                ...
                //var Batchdetails = _context.DataListItems.Where(x => x.DataListId == _context.DataLists.FirstOrDefault(c => c.DataListName.ToLower() == "batch").DataListId.ToString()).ToList();
                ...
                //var feeplans = _context.FeePlans.Where(x => x.ClassId == calssid && x.Batch_Id == activebatch.DataListItemId).ToList();
        ...
        {
            var data = _context.Students.FirstOrDefault(x => x.ApplicationNumber == appnumber && x.IsApprove.ToString() == "191");
            var Classlist = _context.DataListItems.Where(e => e.DataListId == _context.DataLists.FirstOrDefault(x => x.DataListName.ToLower() == "class").DataListId.ToString()).ToList();
            var Category = _context.DataListItems.Where(e => e.DataListId == _context.DataLists.FirstOrDefault(x => x.DataListName.ToLower() == "category").DataListId.ToString()).ToList();
        ...
        public ActionResult RegistrationFeePaymentProcess()
        {
            ...
            if (paymentViewModels != null)
            {
                ...
                var Application = _context.Students.FirstOrDefault(x => x.StudentId == sid).ApplicationNumber;
                var famildetails = _context.FamilyDetails.FirstOrDefault(x => x.ApplicationNumber == Application);
                        ...
                        if (studentdata != null)
                        {
                            if (studentdata.ParentEmail != null)
                            {
                                ...
                                if (createserlogin != null)
                                {
                                    ...
                                    if (userlogin != null)
                                    {
                                        var useremail = SendEmail("" + studentdata.ParentEmail + "", "Regarding Username and Password", "You can login the website using this link" + loginpagelink + ",Username is " + userlogin.UserName + " and password is " + userlogin.Password + ",Your Application has been to the Admitted status successfully");

                                        if (useremail == "S")
                                        {
                                            EmailViewModel emailView = new EmailViewModel();
                                            emailView.Student_id = Convert.ToInt32(studentdata.StudentId);
                                            emailView.ApplicationNumber = studentdata.ApplicationNumber;
                                            emailView.Name = studentdata.Name + " " + studentdata.Last_Name;
                                            emailView.Parent_Email = studentdata.ParentEmail;
        ...
        public ActionResult AdmissionFeepage(int studentId)
        {
            ...
            if (studentdata != null)
            {
                ...
                if (feevalue != null)
                {
                    ...
                    studentsRegistration.EmailId = studentdata.ParentEmail;
        ...
        public ActionResult RegistrationFeepage(int studentRegisterId)
        {
            ...
            var studentdata = _context.Students.FirstOrDefault(x => x.StudentId == studentRegisterId);
            StudentDetailVM studentsRegistration = new StudentDetailVM();
            if (studentdata != null)
            {
        ...
        public ActionResult CapturePayment(PaymentResultModels paymentResultModels)
        {
            ...
            {
                TempData["RedirectUrl"] = Url.Action("FeeReceipts", "AdmissionFee");
        ...
        #region new payment implemention
        //Common Pay
        public ActionResult CommonPaymentprocess(string paymentgatewayName)
        {
            PaymentViewModels paymentViewModels = (PaymentViewModels)TempData["objPaymentViewModels"];
            string orderId = "";
            string TraceID = "";
            ...
            var paymentResultModels = new PaymentResultModels();
            if (paymentViewModels != null)
            {
                paymentResultModels = MapPaymentResultModels(paymentViewModels);
                var studentid = paymentResultModels.studentid;
                ViewBag.PaymentStudentName = paymentResultModels.StudentName;
                ViewBag.PaymentStudentClass = paymentResultModels.Class;
                ViewBag.PaymentStudentSection = paymentResultModels.SectionName;
                ViewBag.PaymentStudentCategory = paymentResultModels.Category;
                ViewBag.PaymentStudentRoleNumber = paymentResultModels.RoleNumber;
                ViewBag.PaymentStudentTCBal = paymentResultModels.TCBal;

                int sid = Convert.ToInt32(studentid);
                var appli=_context.Students.FirstOrDefault(x=>x.StudentId==sid).ApplicationNumber;
                var famildetails = _context.FamilyDetails.FirstOrDefault(x => x.ApplicationNumber == appli);
                paymentResultModels.Contact = famildetails == null ? "" : famildetails.FMobile;
                string PaymentAmount = paymentResultModels.TCBal;
                //string PaymentAmount = transactionamount;
                decimal payAmount = 0;
                payAmount = Convert.ToDecimal(PaymentAmount);
                payAmount = payAmount * 100;
                var keyvalue = GetKeySecretGateWay(paymentResultModels.classdetails, paymentResultModels.FeeHeadings);
                PaymentTransactionId paymentTransactionId = new PaymentTransactionId();
                ...
                if (paymentgatewayName == "Atomic")
                {
                    ...
                    {
                        ViewBag.mer = objPayverify.merchId;
                        ViewBag.tok = objPayverify.atomTokenId;
                        ViewBag.mob = objPayverify.custMobile;
                        ViewBag.Email = objPayverify.custEmail;
                ...
                TempData["paymentgatewayName"] = paymentgatewayName;
                if (ViewBag.error == "")
                {
                    TempData["StudentDetails"] = paymentResultModels;
                    TempData["MerchantDetails"] = paymentTransactionId;
            ...
            //{
            //    userSession[key] = Session[key];
            //}
            
            //TempData["UserSession"] = new Dictionary<string, object>
            //{
            //    { "UserId", Session["UserId"] },
            //    { "ScolarNo", Session["ScolarNo"] },
            //    { "StudentId", Session["StudentId"] },
            //    { "rolename", Session["rolename"] },
            //    { "RoleName", Session["RoleName"] },
            //    { "LoginRoleID", Session["LoginRoleID"] },
            //    { "Name", Session["Name"] },
            //    { "Studentname", Session["Studentname"] },
            //    { "StaffID", Session["StaffID"] },
            //    { "RolepermissionNew", Session["RolepermissionNew"] },
            //    { "rolePermissionList", Session["rolePermissionList"] },
            //    { "MenuList", Session["MenuList"] },
            //    { "SubmenuList", Session["SubmenuList"] },
            //    { "SchoolName", Session["SchoolName"] },
            //    { "SchoolImage", Session["SchoolImage"] }
            //    // Add more keys here if you set more session variables
            //};
            return View(paymentResultModels);
        }
        public Payverify.Payverify PaymentApiCall(PaymentResultModels paymentResultModels)
        {
            try
            {
                ...
                Payrequest.Payrequest objre = new Payrequest.Payrequest();
                ...
                //string testurleq = "https://caller.atomtech.in/ots/aipay/auth?merchId={0}&encData={1}" + Encryptval;
                string testurleq = string.Format(ConfigurationManager.AppSettings["atomtechAuthurl"], ConfigurationManager.AppSettings["atomtechmerchId"], "&", Encryptval);
                System.IO.File.AppendAllText(@"C:\ATUM\RequestData.txt", json + Environment.NewLine);
                System.IO.File.AppendAllText(@"C:\ATUM\RequestDataURL.txt", testurleq + Environment.NewLine);
                ...
                try
                {
                    ...
                    return new Payverify.Payverify();
                }
                catch (Exception ex)
                {
                    return new Payverify.Payverify();
        ...
        [HttpPost]
        public ActionResult CapturePaymentReponse()
        {
            string paymentid = "";
            string orderid = "";
            bool isPaymentstatus = false;
            string paymentgatewayName = TempData["paymentgatewayName"]?.ToString();
            NameValueCollection nvc = Request.Form;
            if (nvc["encdata"] != null)
            {
                paymentgatewayName = "Atomic";
                string encdata = nvc["encdata"];
                EncrypDecrpt encrypDecrpt = new EncrypDecrpt();
                string Decryptval = encrypDecrpt.decrypt(encdata);
                ...
                paymentid = atomTxnId; //gateway 
                orderid = objectres.payInstrument.merchDetails.merchTxnId; //by our system
                isPaymentstatus = objectres.payInstrument.responseDetails.message == "SUCCESS" ? true : false;

                PaymentTransactionId paymentTransactionId = new PaymentTransactionId();
                paymentTransactionId.Paymentid = paymentid;
                paymentTransactionId.Orderid = orderid;
                TempData["Paymentid"] = paymentTransactionId;
                PaymentResultModels paymentViewModels = new PaymentResultModels();
                paymentViewModels.Concession = Convert.ToInt32(objectres.payInstrument.extras.udf3);
                paymentViewModels.StudentName = objectres.payInstrument.extras.udf1;
                paymentViewModels.TCBal = objectres.payInstrument.extras.udf2;
                paymentViewModels.Feeheadingamt = objectres.payInstrument.extras.udf6;
                paymentViewModels.studentid = objectres.payInstrument.extras.udf5;
                TempData["MerchantDetails"] = paymentTransactionId;
                TempData["paymentgatewayName"] = paymentgatewayName;
                TempData["StudentDetails"] = paymentViewModels;
                paymentgatewayName = "Atomic";
            }
            else
            {
                paymentid = Request.Form["razorpay_payment_id"];//Paymentid
                orderid = Request.Form["razorpay_order_id"];//orderid
                string Key = "";
                string Secret = "";
                PaymentTransactionId paymentTransaction = (PaymentTransactionId)TempData["MerchantDetails"];
                if (paymentTransaction != null)
                ...
                string amt = paymentcaptured.Attributes["amount"];
                TempData["Paymentid"] = paymentTransactionId;
                isPaymentstatus = paymentcaptured.Attributes["status"] == "captured" ? true : false;
            }
            TempData["paymentgatewayName"] = paymentgatewayName;
        ...
        public PaymentResultModels MapPaymentResultModels(PaymentViewModels paymentViewModels)
        {
            PaymentResultModels paymentResultModels = new PaymentResultModels();
            var ClassList = _context.DataListItems.Where(e => e.DataListId == _context.DataLists.FirstOrDefault(x => x.DataListName.ToLower() == "class").DataListId.ToString()).ToList();
            var classdetails = ClassList.FirstOrDefault(x => x.DataListItemName == paymentViewModels.Class)?.DataListItemId; //NEW HARISH
            paymentResultModels.StudentName = paymentViewModels.Name;
            paymentResultModels.Class = paymentViewModels.Class;
           ...
           // paymentResultModels.MobileNO = _context.Students.Where(x => x.StudentId == studentid)?.FirstOrDefault()?.Mobile;
            return paymentResultModels;
        }
        public Dictionary<string, string> GetKeySecretGateWay(int classdetails, string FeeHeadings)
        {
        ...
        public ActionResult PaymentSuccessResult()
        {
            ...
            if (paymentgatewayName == "Atomic")
            {
                objinsert_tbl_PaymentTransactionDetails = new PaymentTransactionDetails()
                {
                    ...
                    PaymentId = paymentTransaction.Paymentid,
                    TrackId = paymentTransaction.Orderid,
                    ReferenceNo = paymentTransaction.Orderid,
                    TransactionError = ""
                };
            }
            else
            ...
            if (paymentViewModels != null)
            {
                ...
                _context.SaveChanges();
                genericFeeCalculator(objPaymentStatus, stuid);
                TempData["AlertMessage"] = "Transaction Completed Successfully!";
                HttpCookie cookie = Request.Cookies["UserSessionBackup"];
                if (cookie != null)
                {
                    var sessionData = JsonConvert.DeserializeObject<Dictionary<string, object>>(cookie.Value);
                    foreach (var item in sessionData)
                    {
                        Session[item.Key] = item.Value;
                    }
                }
                //if (TempData["UserSession"] is Dictionary<string, object> userSessionData)
                //{
                //    foreach (var item in userSessionData)
                //    {
                //        Session[item.Key] = item.Value;
                //    }
                //}
                //return RedirectToAction("FeeReceipts", "AdmissionFee");
                return RedirectToAction("Dashboard", "Dashboard");
                //return Content("<script language='javascript' type='text/javascript'>window.location.href='/Dashboard/Dashboard';alert('Transaction Completed Successfully!')</script>");
            }
            else
            {
                TempData["AlertMessage"] = "No Data Can be Displayed......Session is Null";
                if (TempData["UserSession"] is Dictionary<string, object> userSessionData)
                {
                    foreach (var item in userSessionData)
                    {
        ...
        #endregion new payment implemention
    }
