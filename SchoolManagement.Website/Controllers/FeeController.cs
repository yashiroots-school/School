...
namespace SchoolManagement.Website.Controllers
{
    ...
    public class FeeController : Controller
    {
        ...
        #region
        [HttpPost]
        public bool PayDueAmmount(PayDuaAmmountViewModel model)
        {
            if (model == null) return false;

            string className = model.Class;
            float dueAmount = float.Parse (model.DuaAmmount);

            // student के सारे पुराने receipts
            var receipts = _context.TblFeeReceipts
                .Where(x => x.StudentId == model.StudentId && x.ClassName == className && x.OldBalance > 0)
                .OrderBy(x => x.AddedDate)
                .ToList();

            float originalDue = dueAmount;

            foreach (var receipt in receipts)
            {
                if (dueAmount <= 0) break;

                if (dueAmount >= receipt.OldBalance)
                {
                    // पुरा पुराना balance adjust
                    dueAmount -= receipt.OldBalance;
                    receipt.OldBalance = 0;
                    receipt.BalanceAmt = receipt.OldBalance;

                }
                else
                {
                    // partial adjust
                    receipt.OldBalance -= dueAmount;
                    receipt.BalanceAmt = receipt.OldBalance;
                    dueAmount = 0;
                }
            }
    ...
    .ToList();
            ...
            // नया receipt बनाओ
            var newReceipt = new TblFeeReceipts
            {
                StudentId = lastReceipt.StudentId,
                ClassId = lastReceipt.ClassId,
                CategoryId = lastReceipt.CategoryId,
                StudentName = lastReceipt.StudentName,
                ClassName = lastReceipt.ClassName,
                CategoryName = lastReceipt.CategoryName,
                BatchName = lastReceipt.BatchName,
                CurrentYear = lastReceipt.CurrentYear,
                AddedDate = DateTime.Now,
                ModifiedDate = DateTime.Now,
                PaymentMode = lastReceipt.PaymentMode,
                BankName = lastReceipt.BankName,
                CheckId = lastReceipt.CheckId,
                Remark = "Due Payment Adjusted",
                UserId = lastReceipt.UserId,
            ...
            _context.TblFeeReceipts.Add(newReceipt);
            _context.SaveChanges();
